...
- **Every surface that shows a running Agent draws what driving one taught.** This branch was built by seating agents on a real desk, and the roadmap's *Driving an Agent, learned by doing it* records where that failed. For Part B's screens three of those lines are binding: *waiting for a person* is drawn distinctly from *running*, because an unanswered request looks exactly like a hang; the seat a surface shows is the one that actually opened, read back, never the one asked for; and every failure is a sentence with a fix, never *Internal error*.
- **A proof is only a proof if it was run, and a test never moves below the thing it tests.** Every task's report shows each regression test failing against the unfixed code, by disabling the change and watching a named test go red — and says so in those words when a proof could not be run. A shared test helper that changes shape is named in the report and the commit. Both rules come from review of this branch: one claimed proof passed with its guard removed, and a wire test quietly became a handler test beneath the validator it existed to check.
