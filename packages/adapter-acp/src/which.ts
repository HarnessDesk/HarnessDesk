 ...
 *
 * It lives in three packages, byte for byte: `packages/server/src/installs/`,
 * `packages/adapter-acp/src/` and `packages/codex/src/`, each `which.ts`,
 * because none of them may import another, and `packages/codex` imports no
 * other package at all. A test in `packages/server` holds the three equal,
 * so a change is made in one and copied (#129).
 */
