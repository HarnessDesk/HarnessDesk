...
test('an ACP runtime reports the provider its reader resolved, and unknown without one', () => {
  const asked: (string | undefined)[] = []
  const read = new AcpRuntime({
    id: 'reader-acp', name: 'Reader', command: 'true',
    resolveProvider: (cwd) => { asked.push(cwd); return cwd === '/elsewhere' ? null : 'vendor-a' },
  })
  assert.equal(read.info.provider, 'vendor-a')
  assert.equal(read.providerAt('/project'), 'vendor-a')
  assert.equal(read.providerAt('/elsewhere'), null, 'a folder whose own configuration overrides it')
  ...
  assert.equal(unread.providerAt('/project'), null)
})
