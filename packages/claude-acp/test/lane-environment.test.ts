...
test('bridge parses only negotiated metadata and preserves response metadata', () => {
  ...
  assert.deepEqual(
    environmentAck({ _meta: { vendor: true, harnessdesk: { options: 'kept' } } }, env(30_000)),
    {
      _meta: {
        vendor: true,
        harnessdesk: { options: 'kept', environment: env(30_000) },
      },
    },
  )
})
...
test('the actual bridge child gets both environments and re-applies one after restart', async (t) => {
  ...
  // The scripted CLI does not write Claude's on-disk transcript store. Give
  // the restarted bridge the same minimal synthetic row the real CLI leaves,
  // so ACP can safely recover the session's cwd before sending session/load.
  const project = join(home, 'config', 'projects', home.replace(/[^a-zA-Z0-9]/g, '-'))
  await mkdir(project, { recursive: true })
  await writeFile(
    join(project, `${a.id}.jsonl`),
    `${JSON.stringify({
      isSidechain: false,
      sessionId: String(a.id),
      cwd: home,
      type: 'user',
      message: { role: 'user', content: 'Read the fixture.' },
    })}\n`,
  )
  await first.dispose()
  second = make()
