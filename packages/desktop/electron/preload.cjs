...
contextBridge.exposeInMainWorld('harnessdesk', {
  ...
  browserReady: (request) => {
    if (
      Number.isInteger(request?.webContentsId) &&
      (request.profile === null || typeof request.profile === 'string')
    ) {
      ipcRenderer.send('harnessdesk:browser-ready', {
  ...
  browserFocused: (request) => {
    if (Number.isInteger(request)) {
      ipcRenderer.send('harnessdesk:browser-focused', { request })
    }
  },
  onBrowserShow: (handler) => {
    ...
    const listener = (_event, request) =>
      handler({
        ...
        url: typeof request?.url === 'string' ? request.url : 'about:blank',
      })
  ...
  onBrowserFocus: (handler) => {
    if (typeof handler !== 'function') return () => {}
    const listener = (_event, request) =>
      handler({ profile: request?.profile ?? null, request: request?.request })
  ...
  clearBrowserData: (profile = null) =>
    ipcRenderer.invoke('harnessdesk:browser-clear-data', { profile }),
  /**
  ...
  onBrowserOpenTab: (handler) => {
    ...
    const listener = (_event, request) => {
      if (typeof request?.url === 'string') {
        handler({ url: request.url, profile: request.profile ?? null })
      }
    }
