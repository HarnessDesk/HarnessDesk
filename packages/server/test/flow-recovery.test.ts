...
test('a relaunch whose Seat was closed stalls with why, never reads as running over nobody (#915)', async (t) => {
  ...
  assert.equal(run!.state, 'stalled')
  assert.match(run!.reason ?? '', /The Seat for card #1 is closed, so its card was not handed back after the desk restarted\. Seat the Agent again to pick it up\./)
...
test('a Seat closed while its run is held by a pause or the cap stalls with why on release, never reads as running over nobody (#939)', async (t) => {
  ...
  assert.equal(run!.reason, `The Seat for card #${card.id} is closed, so its card was not handed back. Seat the Agent again to pick it up.`)
