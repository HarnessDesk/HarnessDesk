...
const Row = ({
  ...
  return (
    <div className={`${styles.row} rounded-(--hd-radius) bg-(--hd-card) shadow-(--hd-hairline)`}>
      ...
      {collapsible && open && (
        <div className={bareBody ? "my-(--hd-space-0-5) mb-(--hd-space-1-5) ms-(--hd-space-6)" : `py-(--hd-space-2) px-(--hd-space-3) ps-(--hd-space-3) pe-(--hd-space-3) pb-(--hd-space-3) ps-(--hd-space-6)`}>{children}</div>
      )}
    </div>
  )
}
