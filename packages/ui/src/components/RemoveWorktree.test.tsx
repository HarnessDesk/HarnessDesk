...
it('removes a clean worktree without asking to discard anything', async () => {
  const { store, onClose } = await rig({ modified: 0, untracked: 0, unpushedCommits: 1, files: [] })
...
it('names every file a removal would discard before it offers to discard them', async () => {
  const { store } = await rig({ modified: 1, untracked: 1, unpushedCommits: 0, files: ['src/parser.ts', 'notes.md'] })
