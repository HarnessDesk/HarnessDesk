...
import { Button, Checkbox, Field, Input, NativeSelect, Row, Rows, SectionHead, Textarea } from '../design'
...
export const ShapeStep = ({ role, agents, onChange, onRemove }: ShapeStepProps) => {
  ...
  return (
    <div>
      <Field label="Id" hint="What rules and the seed name this step by. Renaming updates every place that names it.">
      ...
      {role.kind === 'agent' && (
        <>
          ...
          <Field label="Independent of" hint="Other Agent steps, one id per line — a provider guard the compiler checks, never a runtime comparison.">
            {(control) => (
              <Textarea
                {...control}
                rows={2}
      ...
      <Button variant="outline" size="sm" onClick={onRemove}>Remove this step</Button>
    </div>
  )
}
