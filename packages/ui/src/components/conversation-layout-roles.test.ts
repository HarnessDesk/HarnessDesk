...
import turnWorkPatternCss from '../design/patterns/TurnWork.module.css?raw'
...
it('does not let a work-header hover erase trouble ink', () => {
  expect(turnWorkPatternCss).toMatch(/\.head:not\(\[data-trouble\]\):hover\s+\.headLabel\s*\{[^}]*color:\s*var\(--hd-secondary-foreground\)/s)
