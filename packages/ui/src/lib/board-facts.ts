import type { CardEvidence, EvidenceView, FlowExecution, FlowRole, FlowRun, Intent } from '@harnessdesk/protocol'
...
export const placeCard = ({ intent, evidence, stranded, holderWaits, forPerson }: PlaceInput): Placement => {
  switch (intent.state) {
    ...
    case 'blocked':
      return intent.blockedBy === 'hand'
        ? { column: 'needs', why: 'stopped' }
        : { column: 'todo', why: null }
    case 'open':
