...
beforeEach(() => {
  ...
  vi.spyOn(store.transport, 'request').mockImplementation((async (
    ...
    if (method === 'credentials/list') {
      return [{ ref: 'cred_a', name: 'Proxy key', createdAt: 1_700_000_000_000, owner: null }]
    }
...
it('forgetting a stored key names it by reference, and says whether it went', async () => {
  ...
  expect(keys).toEqual([{ ref: 'cred_a', name: 'Proxy key', createdAt: 1_700_000_000_000, owner: null }])
